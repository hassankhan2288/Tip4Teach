    ...
    <div class="container text-center">
        <div class="row">
            <div class="col-md-6 text-right">
                <form method="POST" action="{{ route('admindownload.PDF', ['id' => $sale->id]) }}">
                    @csrf
                    <input type="hidden" name="htmlContent" id="htmlContent">
                    <button type="submit" class="btn btn-primary btn btn-primary m-0"><i
                            class="far fa-file-pdf"></i>PDF Speichern</button>
                </form>
            </div>
            <div class="col-md-6 text-left">
                <button onclick="window.print()" class="btn btn-primary m-0"><i class="fas fa-print"></i>
                    Drucken</button>
            </div>
        </div>
    </div>
    ...
    <section class="p-t-10">
        <div class="container" style="background-color: #fff; background-color: #fff;padding: 30px; max-width: 980px;">
            ...
            <table class="w-100 mt-4">
                ...
                {{-- <tr>
                    <td></td>
                    @if ($sale->branch != null && $sale->branch->company_name != null)
                        <td class="col-md-4 px-0">
                            {{ $sale->branch->company_name }}
                        </td>
                    @elseif ($sale->customer != null && $sale->customer->company_name != null)
                        <td class="col-md-4 px-0">
                ...
                <tr>
                    <td class="col-md-4 px-0">
                        ...
                        {{ $sale->shipping_address->company_name ?? '' }}
                        @else
                        {{ $sale->billing_address->company_name ?? '' }}
                        @endif
                    </td>
                    <td class="col-md-4 px-0">
            ...
            @if ($sale['comment'] != null)
                <hr style="border: 1px solid #dbd9d9;">
                <tr style="vertical-align: top;border-top: 1px solid #d1d1d1;">
                    <td colspan="2"
                        style="word-break: break-word;border-collapse: collapse !important;vertical-align: top;padding: 10px 15px;">
                        <table role="presentation" cellpadding="0" cellspacing="0" width="100%" border="0">
                            <tr>
                                <td>
                                    <p style="color: #333333;">
                                        <strong>Bemerkungen : </strong>{{ $sale['comment'] }}
                                    </p>
                                </td>
                            </tr>
                        </table>
            ...
            <table class="tabledata"
                ...
                <thead>
                    <tr>
                        ...
                        <th colspan="5" style="padding:6px;text-align:right">Netto</th>
                    </tr>
                </thead>
                <tbody>
                    @if ($items)
                        @foreach ($items as $item)
                            <tr>
                                ...
                                <td style="padding:6px;text-align:right" valign="top"><i
                                    ...
                                    {{ currency_format($item->product->price, 2) }}
							...
							{{ currency() }}{{ number_format($item->tax, 2) }}</td> --}}
                                <td colspan="5" style="padding:6px;text-align:right" valign="top"><i
                                    ...
                                    {{ currency_format($item->product->price * $item->qty, 2) }}</td>
                            </tr>
                        @endforeach
                    @endif
                </tbody>
                <tfoot>
                    ...
                    <tr>
                        ...
                        @if ($sale->order_discount != 0)
                            <td style="text-align:right;font-size:14px;font-weight:bold">
                                -{{ currency_format($sale->order_discount, 2) }}</td>
                        @else
                            <td style="text-align:right;font-size:14px;font-weight:bold">
                                {{ currency_format($sale->order_discount, 2) }}</td>
                        @endif
                    ...
                    <tr>
                        ...
                        <td style="text-align:right;font-size:14px;font-weight:bold">
                            {{ currency_format($sale->order_tax, 2) }}</td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
